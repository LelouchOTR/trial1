...
export default async function bulkUploadToElastic(formatted_data) {
  ...
  if (exists1) { // If the index exists
    await client.indices.delete({ index: 'temp_index' }); // Delete the index
    console.log("index deleted");
    await client.indices.create({ // Create a new index with the same name and our required mappings
      index: 'temp_index',
      operations: {
        mappings: {
          properties: {
  ...
  const bulkResponse = await client.bulk({ refresh: true, body: formatted_data_json }) // Bulk push the data to the elasticsearch node

  if (bulkResponse.errors) { // If there are any errors in the bulk response
    const erroredDocuments = [] // Get the operation name (index, update, delete, etc.)
    // The items array has the same order of the dataset we just indexed.
    // The presence of the `error` key indicates that the operation
    // that we did for the document has failed.
    bulkResponse.items.forEach((action, i) => { // Loop through the items array
      const operation = Object.keys(action)[0] // Get the operation name (index, update, delete, etc.)
      if (action[operation].error) { // If the operation has an error
